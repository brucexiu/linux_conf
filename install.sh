...
function command_install()
{
    if [[ $1 == ubuntu ]]  
    then
        for package in libxslt  libffi gcc  openssl jq libxml2-dev libxslt1-dev python-dev tmux parallel ctags
        do
    ...
    then
        yum install libxslt-devel  libffi libffi-devel  python-devel gcc  openssl openssl-devel mysql-devel jq tmux ctags
    fi
}
